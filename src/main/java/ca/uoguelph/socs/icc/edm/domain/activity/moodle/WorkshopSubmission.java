...
public class WorkshopSubmission extends GenericGroupedActivityMember
{
	...
	private static final class Factory implements ActivityGroupMemberElementFactory
	{
		...
		{
			if (! (parent instanceof Workshop))
			{
				throw new IllegalArgumentException ("Parent is not an instance of Workshop");
			}
		...
		{
			((WorkshopSubmission) activity).setId (id);
		}
	}
	...
	/**
	 * Register the <code>WorkshopSubmission</code> with the factories on initialization.
	 */
	...
	public WorkshopSubmission (ActivityGroup parent, String name)
	{
		super (parent, name);
	}
