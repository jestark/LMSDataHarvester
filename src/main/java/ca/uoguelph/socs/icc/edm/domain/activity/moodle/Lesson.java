...
public class Lesson extends GenericGroupedActivity
{
	/**
	...
	private static final class Factory extends AbstractNoIdElementFactory<Activity> implements NamedActivityElementFactory, ActivityGroupElementFactory
	{
		...
		{
			if (! (member instanceof LessonPage))
			{
				throw new IllegalArgumentException ("Child is not a member of LessonPage");
			}

			return ((Lesson) group).addChild (member);
		}
		...
		public boolean removeChild (ActivityGroup group, ActivityGroupMember member)
		{
			...
			return ((Lesson) group).removeChild (member);
		}
	}
	...
	/**
	 * Register the <code>Lesson</code> with the factories on initialization.
	 */
	 ...
	 * to separate the instances of <code>Lesson</code> from the other
